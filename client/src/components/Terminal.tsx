...
└──────────────────────────────────────────────┘
  ...
  const executeCommand = useCallback(async () => {
    ...
    if (commands[cmd]) {
      ...
      await typeOutput(
        `Command not found: ${cmd}\nType 'help' or 'commands' for available commands.`,
        "error",
      );
    }
  ...
  return (
    <div
      className="h-full flex flex-col max-w-full terminal-bg"
      onClick={handleClick}
      ...
      <div className="cyber-border scan-line p-2 sm:p-3 bg-gradient-to-r from-black via-gray-900 to-black">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            <span className="terminal-muted text-[10px] sm:text-xs italic mr-2 hidden sm:inline-block">
              — Crafted by Bilal Siki © 2024 —
            </span>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className="terminal-accent text-xs sm:hidden">◉</span>
            ...
            {/* Last updated label */}
            <span
              className="terminal-muted text-[10px] sm:text-xs ml-2 italic"
              title="Last profile update"
            >
              Last updated:{" "}
              {new Date("2024-06-09T23:00:00").toLocaleDateString()}{" "}
        ...
        {terminalState.output.map((line, index) => (
          <div key={index} className="mb-1 sm:mb-2">
            {line.type === "command-title" ? (
              ...
              <>
                <pre
                  className={`hidden sm:block whitespace-pre-wrap font-mono text-xs sm:text-sm leading-relaxed ${
                    line.type === "error"
                      ? "terminal-error"
                      : line.type === "prompt"
                        ? "terminal-prompt"
                ...
                <pre
                  className={`sm:hidden whitespace-pre-wrap font-mono text-xs leading-relaxed ${
                    line.type === "error"
                      ? "terminal-error"
                      : line.type === "prompt"
                        ? "terminal-prompt"
              ...
              <pre
                className={`whitespace-pre-wrap font-mono text-xs sm:text-sm leading-relaxed ${
                  line.type === "error"
                    ? "terminal-error"
                    : line.type === "prompt"
                      ? "terminal-prompt"
